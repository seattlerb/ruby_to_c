...
class TestParseTree < Test::Unit::TestCase
  ...
  @@stupid = [:defn, "stupid", [:scope, [:block, [:args], [:return, [:nil]]]]]
  @@simple = [:defn, "simple", [:scope, [:block, [:args, "arg1"], [:fcall, "print", [:array, [:lvar, "arg1"]]], [:fcall, "puts", [:array, [:call, [:lit, 4], "+", [:array, [:lit, 2]]]]]]]]
  @@global = [:defn, "global", [:scope, [:block, [:args], [:call, [:gvar, "$stderr"], "puts", [:array, [:str, "blah"]]]]]]
  @@lasgn_call = [:defn, "lasgn_call", [:scope, [:block, [:args], [:lasgn, "c", [:call, [:lit, 2], "+", [:array, [:lit, 3]]]]]]]
  @@conditional1 = [:defn, "conditional1", [:scope, [:block, [:args, "arg1"], [:if, [:call, [:lvar, "arg1"], "==", [:array, [:lit, 0]]], [:return, [:lit, 1]], nil]]]]
  @@conditional2 = [:defn, "conditional2", [:scope, [:block, [:args, "arg1"], [:if, [:call, [:lvar, "arg1"], "==", [:array, [:lit, 0]]], nil, [:return, [:lit, 2]]]]]]
  @@conditional3 = [:defn, "conditional3", [:scope, [:block, [:args, "arg1"], [:if, [:call, [:lvar, "arg1"], "==", [:array, [:lit, 0]]], [:return, [:lit, 3]], [:return, [:lit, 4]]]]]]
  @@conditional4 = [:defn, "conditional4", [:scope, [:block, [:args, "arg1"], [:if, [:call, [:lvar, "arg1"], "==", [:array, [:lit, 0]]], [:return, [:lit, 2]], [:if, [:call, [:lvar, "arg1"], "<", [:array, [:lit, 0]]], [:return, [:lit, 3]], [:return, [:lit, 4]]]]]]]
  ...
  @@multi_args = [:defn, "multi_args", [:scope, [:block, [:args, "arg1", "arg2"], [:fcall, "puts", [:array, [:call, [:lvar, "arg1"], "*", [:array, [:lvar, "arg2"]]]]], [:return, [:str, "foo"]]]]]
  @@bools = [:defn, "bools", [:scope, [:block, [:args, "arg1"], [:if, [:call, [:lvar, "arg1"], "nil?"], [:return, [:false]], [:return, [:true]]]]]]
